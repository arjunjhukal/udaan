...
export const ticketApi = baseApi.injectEndpoints({
	...
	endpoints: (builder) => ({
		...
		createTicketType: builder.mutation<GlobalResponse, { name: string }>({
			query: (body) => ({ url: "/ticket/ticket-type", method: "POST", body }),
			invalidatesTags: [{ type: "TicketType", id: "LIST" }],
		}),

		updateTicketType: builder.mutation<GlobalResponse, { id: number; name: string }>({
			query: ({ id, name }) => ({ url: `/ticket/ticket-type/${id}`, method: "PUT", body: { name } }),
			invalidatesTags: (_result, _error, { id }) => [
				{ type: "TicketType", id },
				{ type: "TicketType", id: "LIST" },
			],
		...
		deleteTicketType: builder.mutation<GlobalResponse, { ids: number[] }>({
			query: ({ ids }) => ({
				url: "/ticket/ticket-type",
				method: "DELETE",
				body: { ticket_types: ids },
			}),
			invalidatesTags: [{ type: "TicketType", id: "LIST" }],
		}),
