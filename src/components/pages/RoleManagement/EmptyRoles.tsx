...
export default function EmptyRoles() {
    ...
    return (
        <div className="empty__roles flex justify-center items-center h-full">
            <div className="empty_content flex justify-center items-center flex-col gap-6 max-w-[444px] text-center">
                ...
                <div className="content">
                    <Typography variant="h4" className="mb-2!">{t("messages.empty_states.roles.title")}</Typography>
                    <Typography variant="subtitle1">{t("messages.empty_states.roles.description")}</Typography>
                </div>
                <Button variant="contained" color="primary" startIcon={<Add />} onClick={() => {
                    navigate(PATH.ROLES.CREATE_ROLE.ROOT)
                }}>{t("messages.empty_states.roles.action")}</Button>
            </div>
        </div>
    )
}
