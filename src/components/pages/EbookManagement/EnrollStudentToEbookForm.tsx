...
import { useEnrollStudentToEbookMutation } from "../../../services/ebookApi";
...
export default function EnrollStudentToEbookForm({ open, setOpen, id }: Props) {
    ...
    const [enrollStudent, { isLoading: enrollingStudent }] = useEnrollStudentToEbookMutation();
    ...
    const formik = useFormik({
        initialValues: { student_id: null as number | null },
        validationSchema,
        ...
        onSubmit: async (values) => {
            try {
                ...
                dispatch(showToast({
                    message: response?.message || "eBook assigned successfully",
                    severity: "success",
                }));
                handleClose();
            } catch (e) {
                dispatch(showToast({
                    message: getApiErrorMessage(e, "Unable to assign eBook to student."),
                    severity: "error",
                }));
            }
        },
                ...
                <form onSubmit={formik.handleSubmit} className="flex flex-col gap-6">
                    <div className="header">
                        <div className="flex items-center justify-between">
                            <Typography variant="h5">Assign eBook to Student</Typography>
                    ...
                    <Box className="flex justify-end gap-4 py-6 sticky bottom-0 left-0 right-0" bgcolor={theme.palette.primary.contrastText}>
                        ...
                        <Button variant="contained" type="submit" disabled={enrollingStudent}>
                            {enrollingStudent ? "Assigning..." : "Assign Now"}
                        </Button>
                    </Box>
                </form>
            </DialogContent>
