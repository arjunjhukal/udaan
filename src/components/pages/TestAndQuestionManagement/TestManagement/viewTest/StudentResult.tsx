...
export default function StudentResult({ id }: { id: string }) {
    ...
    const { data, isLoading } = useGetListOfStudentSubmittedTestQuery({ id: Number(id), qp, search }, { skip: !id });
    ...
    const isSomeSelected = selectedRows.size > 0 && selectedRows.size < results.length;
    ...
    const columns = useMemo<ColumnDef<StudentSubmitTestProps>[]>(() => [
        ...
        },
        {
            ...
            accessorKey: "name",
            cell: ({ row }) => (
                <Typography variant="subtitle1">{row.original?.student?.name}</Typography>
            ),
        },
        {
            header: "Answered",
            ...
            cell: ({ row }) => (
                <div className="flex justify-start items-center">
                    <Typography variant="subtitle1" color="text.dark">{row.original?.total_attempted}</Typography>

                    <Typography variant="subtitle1" color="text.middle">/{row.original?.total_questions}</Typography>
                </div>
            ),
        },
        {
        ...
        },
        {
            ...
            accessorKey: "timer",
            cell: ({ row }) => (
                <Typography variant="subtitle1">{row.original?.timer}</Typography>
            ),
        },
        {
            header: "Status",
            accessorKey: "status",
            cell: ({ row }) => (
                <Typography variant="subtitle1" className="capitalize" color={row.original.status === "progress" ? "error.main" : "success.main"}>{row.original?.status}</Typography>
            ),
        },
        {
            header: "Result",
            accessorKey: "result",
            cell: ({ row }) => (
                <Typography variant="subtitle1" className="capitalize flex items-center justify-center rounded-md p-1" color={row.original.result === "failed" ? "error.main" : "success.main"} bgcolor={row.original.result === "progress" ? "error.light" : "success.light"}>
                   {row.original.status==="" <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    {row.original?.result}</Typography>
            ),
        },
        {
            header: "Total Marks",
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <ActionIconVisible
                    onView={() => navigate(PATH.TEST_QUESTION_MANAGEMENT.TEST.CHECK_PAPER.ROOT(row.original.id))}
                />
            ),
        },
    ...
    return (
        <div className="students__attended__test mt-8">
            <TableFilter
                ...
                onFilter={() => { }}
            />
            {!isLoading && !results.length ? <EmptyRoute
