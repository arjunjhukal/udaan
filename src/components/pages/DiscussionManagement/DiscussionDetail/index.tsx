...
import { useDeleteDiscussionMutation, useGetDiscussionByIdQuery, useToggleDiscussionVisibilityMutation } from "../../../../services/discussionApi";
		...
		<div className="discussion__detail__root h-full flex flex-col">
			...
			<Box className="detail__body flex-1 py-4" sx={{ maxWidth: 860 }}>
				...
				<Stack className="mb-4 flex-wrap gap-2" direction="row" alignItems="center" justifyContent="space-between">
					...
					<CAN permissions={["edit_discussions", "delete_discussions"]}>
						<ActionIconVisible
							onEdit={() => navigate(PATH.DISCUSSION.EDIT.ROOT(discussion.id))}
							onDelete={() => setOpenConfirm(true)}
							onTrash={handleToggleVisibility}
						/>
						{/* <Stack direction="row" gap={1}>
							<CAN permissions={["add_discussions"]}>
								<Button variant="outlined" size="small" >
									{discussion.status === "visible" ? t("actions.hide") : t("actions.show")}
								</Button>
							</CAN>
							<CAN permissions={["edit_discussions"]}>
								<Button
									variant="contained"
									size="small"
								>
							...
							<CAN permissions={["delete_discussions"]}>
								<Button variant="outlined" color="error" size="small" onClick={() => setOpenConfirm(true)}>
									{t("actions.delete")}
								</Button>
							</CAN>
						</Stack> */}
				...
				<Box
					className="discussion__content general__content__box styled__list prose max-w-none"
					dangerouslySetInnerHTML={{ __html: discussion.description }}
				/>
				<CommentSection discussionId={discussion.id!} />
			</Box>

			{/* <div className="detail__footer pt-4">
				<Button
					variant="outlined"
					startIcon={<ArrowLeft />}
