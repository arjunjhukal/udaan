...
import { Dislike, Like1 } from "iconsax-reactjs";
...
import { useCreateReplyMutation, useDeleteCommentMutation, useDislikeCommentMutation, useLikeCommentMutation, useSuspendCommentMutation, useUpdateCommentMutation } from "../../../../services/commentApi";
...
export default function CommentItem({ comment, discussionId }: Props) {
	...
	const [suspendComment] = useSuspendCommentMutation();
	...
	const handleSuspend = async () => {
		if (!comment.id) return;
		const newStatus = comment.status === "active" ? "suspended" : "active";
		try {
			const res = await suspendComment({ id: comment.id, status: newStatus, discussion_id: discussionId }).unwrap();
			dispatch(showToast({ message: res.message || t("messages.comment.status_updated"), severity: "success" }));
		} catch (e: any) {
			dispatch(showToast({ message: e?.data?.message || t("messages.comment.status_error"), severity: "error" }));
		}
	};
			...
			<Stack direction="row" gap={2}>
				...
				<Box className="flex-1 min-w-0">
					...
					<Stack direction="row" alignItems="center" justifyContent="space-between" gap={1}>
						...
						{/* <CAN permissions={["edit_comments", "delete_comments", "suspend_comments"]}> */}
						<Actions
							onEdit={() => setIsEditing(true)}
							onDelete={() => setOpenDeleteConfirm(true)}
							onSuspend={handleSuspend}
							userStatus={comment.status === "suspended"}
						/>
						{/* </CAN> */}
					</Stack>
