...
import { useDeleteReplyMutation, useSuspendReplyMutation, useUpdateReplyMutation } from "../../../../services/commentApi";
...
export default function ReplyItem({ reply, discussionId }: Props) {
	...
	const handleSuspend = async () => {
		if (!reply.id) return;
		const newStatus = reply.status === "active" ? "suspended" : "active";
		try {
			const res = await suspendReply({ id: reply.id, status: newStatus, discussion_id: discussionId }).unwrap();
			...
			<Box className="flex-1 min-w-0">
				<Stack direction="row" alignItems="center" justifyContent="space-between" gap={1}>
					...
					<Actions
						...
						userStatus={reply.status === "suspended"}
					/>
					{/* </CAN> */}
				</Stack>
