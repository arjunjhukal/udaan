...
export default function AllUserTable() {
    ...
    const openDeleteConfirmation = (selectedUserIds: string[]) => {
        ...
        setUsersToSuspend(selectedUserIds);
        setOpenConfirm(true);
    };
    ...
    const handleUserSuspension = async () => {
        try {
            ...
            dispatch(
                showToast({
                    message: response.message || "User suspended successfully.",
                    severity: "success",
                })
            );
            setSelectedRows(new Set());
            ...
            dispatch(
                showToast({
                    message: e?.data?.message || "Unable to suspend user.",
                    severity: "error",
                })
            );
            setOpenConfirm(false);
            ...
            cell: ({ row }) => (
                <Box className="flex">
                    <Actions
                        ...
                        onSuspend={() => openDeleteConfirmation([row.original.id?.toString() || ""])}
                        userStatus={row.original.is_suspended}
                    />
                    {row.original.is_suspended ? <IconButton
                        className="activate"
                        onClick={() => openDeleteConfirmation([row.original.id?.toString() || ""])}
                    >
    ...
    return (
        <>
            ...
            <ConfirmationDialog
                ...
                title={usersToSuspend ? "Suspend User" : "Delete User"}
                description={usersToSuspend ? "Are you sure you want to delete the selected user(s)? This action can be undone." : "Are you sure you want to delete the selected user(s)? This action cannot be undone."}
                onSave={usersToSuspend ? handleUserSuspension : handleDeleteUser}
