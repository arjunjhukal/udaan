...
import { useGetEnrollmentAnalyticsQuery } from '../../../../services/courseApi';
import AnalyticsCard from '../../../organism/Cards/AnalyticsCard';

export default function EnrollmentAnalytics() {
  const { data } = useGetEnrollmentAnalyticsQuery();
  ...
  return (
    <div className="flex flex-col gap-4 sm:grid sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 mb-6 px-1">
      {
        analytics.map((item) => (
          <Paper sx={{
