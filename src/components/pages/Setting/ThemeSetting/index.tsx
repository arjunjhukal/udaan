...
export default function ThemeSettingRoot() {
    ...
    const formik = useFormik<ThemeSettingFormProps>({
        ...
        onSubmit: async (values) => {
            ...
            try {
                ...
                dispatch(showToast({ message: response?.message || "Theme updated successfully", severity: "success" }));
            } catch (e: any) {
                dispatch(showToast({ message: e?.data?.message || "Unable to update theme", severity: "error" }));
            }
        },
    });
    ...
    return (
        <form onSubmit={formik.handleSubmit} className="app__settings__page__root pb-4 lg:pb-6">
            <Typography variant="h5">{t("messages.theme_settings") || "Theme Settings"}</Typography>
            ...
            <div className="text-right">
                <Button type="submit" variant="contained" disabled={isLoading}>
                    {data?.data ? (isLoading ? "Updating..." : "Update Theme") : "Save Theme"}
                </Button>
            </div>
        </form>
    );
