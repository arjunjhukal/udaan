...
export default function ImportQuestion({
    ...
    if (!questions.length) {
        return (
            ...
            >
                ...
                >
                    {isLoading ? (
                        <LinearProgress sx={{ width: 32, height: 32 }} />
                    ) : (
    ...
    return (
        <form onSubmit={formik.handleSubmit}>
            <Box
                className="flex flex-col justify-start items-start gap-3 p-3 border rounded-lg"
                sx={{ borderColor: theme.palette.separator.dark, backgroundColor: theme.palette.background.paper }}
            >
                {questions.length ? questions.map((question, index) => (
                    <Box className="question__box pb-4 mb-4 lg:pb-8 lg:mb-8 border-b last:border-b-0 last:mb-0 last:pb-0" key={question.id} sx={{ borderColor: (theme) => theme.palette.separator.dark }}>
                        <div className="flex justify-between items-center">
                            <Typography className="mb-6!" variant="body2">Question {index + 1} of {questions?.length}</Typography>
                        </div>
