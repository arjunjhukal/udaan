...
export default function ActionIconVisible({ onDelete, onEdit, onView }: { onDelete?: () => void, onEdit?: () => void, onView?: () => void }) {
    const theme = useTheme();
    return (
        <div className="action__group flex justify-end gap-3">
            {onEdit ? <IconButton className="p-1.5 rounded-md!" sx={{
            ...
            {onDelete ? <IconButton className="p-1.5 rounded-md!" sx={{
                border: `1px solid ${theme.palette.separator.darker}`,
                background: theme.palette.primary.contrastText,
                "&:hover": {
                    color: theme.palette.primary.contrastText,
