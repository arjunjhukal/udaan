...
export default function DiscussionCard({ data, onDelete, onToggleVisibility }: Props) {
	...
	return (
		...
		>
			<Stack direction="row" justifyContent="space-between" alignItems="flex-start" mb={1}>
				...
				<CAN permissions={['edit_discussions', 'delete_discussions', 'hide_discussions']}>
					<Box onClick={(e) => e.stopPropagation()} sx={{ ml: 1, flexShrink: 0 }}>
						<Actions
							editUrl={PATH.DISCUSSION.EDIT.ROOT(data.id)}
							onDelete={onDelete}
							onSuspend={onToggleVisibility}
							userStatus={data.status === 'hidden'}
						/>
					</Box>
				</CAN>
			</Stack>
